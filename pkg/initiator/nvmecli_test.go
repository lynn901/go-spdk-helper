...
func TestListRecognizedNvmeDevicesFromSysfs(t *testing.T) {
	t.Run("sysfs block directory layout", func(t *testing.T) {
		...
		// Mock metadata for nvme0n1: 10GiB, hw_sector_size 512, nsid 7
		nvme0n1Dir := filepath.Join(tmpDir, "nvme0n1")
		if err := os.WriteFile(filepath.Join(nvme0n1Dir, "size"), []byte("20971520\n"), 0644); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(nvme0n1Dir, "nsid"), []byte("7\n"), 0644); err != nil {
			t.Fatal(err)
		}
		...
		if d0.NameSpace != 7 {
			t.Errorf("expected NSID 7 from sysfs nsid file for nvme0n1, got %d", d0.NameSpace)
		}
